...
class PendWindow(tk.Frame):
    ...
    def __init__(self, parent):
        ...
        lblLen = tk.Label(self, text="Length of Pendulum (M):")

        # Create a text field where the user will enter her age.
        txtLen = FloatEntry(self, .1, 1000, width=5)

        # Create a label that displays "Rates:"
        lblSwing = tk.Label(self, text="Swing Time:")

        # Create labels that will display the output.
        lblTime = tk.Label(self, width=10)
        ...
        lblLen.grid(  row=0, column=0, padx=3, pady=2)
        txtLen.grid(  row=0, column=1, padx=3, pady=2)
        lblSwing.grid(row=0, column=2, padx=(30,3), pady=2)
        lblTime.grid( row=0, column=3, padx=3, pady=2)
        btnClear.grid(row=1, column=0, padx=3, pady=2, columnspan=5, sticky="W")

        self.master.title("Swing of the Pendulum")
        ...
        def calc(event):
            ...
            try:
                ...
                swing = round(2 * math.pi * math.sqrt(length / 9.81), 2)


                # Display the slowest and fastest benficial
                # heart rates for the user to see.
                lblTime.config(text=str(f'{swing} Seconds'))
